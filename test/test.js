...
var sendPackageJson = function(req, res) {
...
var sendReposArray = function(req, res) {
  res.end(JSON.stringify([
    {
      name: 'test repo',
      fork: true,
...
describe('Init function', function() {
  ...
  it('Should get connection refused on request when not listening', function(done) {
    c({url: url, user: 'eiriksm'}, function(e) {
  ...
  it('Should init and loop through repos when given a good response', function(done) {
    ...
    c({url: url, user: 'eiriksm', rawurl: 'bogus'}, function(e) {
  ...
  it('Should return something more useful when we also return a package json', function(done) {
    serverResponse = function(req, res) {
      if (req.url === '/colors') {
        ...
        /*eslint-disable key-spacing,comma-spacing,quotes */
        ...
        /*eslint-enable key-spacing,comma-spacing,quotes */
        ));
        return;
      }
      if (req.url.indexOf('page=2') > 0) {
...
describe('Random tests to increase test coverage', function() {
  ...
  it('Should fail miserably with bogus rawurl', function(done) {
    c({url: url, user: 'eiriksm', rawurl: 'bogus://this.will not work at all'}, function(e) {
      e.message.should.equal('No more tips to show!');
      done();
    });
  });
  ...
  it('Should use cache when present', function(done) {
    var id = 'http://localhost:9876/users/eiriksm/repos?page=1';
    require('../lib/cache').set(id, {error: new Error('test error')});
    c({url: url, debug: true, rawurl: url + '.package.json', user: 'eiriksm', npm: {registry: 'http://localhost:9876'}}, function(e) {
